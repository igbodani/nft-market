...
DROP PROCEDURE IF EXISTS CreateToken;
...
DROP PROCEDURE IF EXISTS GetAllTokens;
...
CREATE PROCEDURE CreateContract(IN _address varchar(100),  IN _name varchar(50), IN id int(11), IN _symbol varchar(20),
                                IN _type varchar(8), IN _owner varchar(75), IN _dateCreated datetime,
                                IN _dateModified datetime, IN _dateDeleted datetime)
BEGIN
    INSERT INTO contracts (address, name,tokenId, symbol, type, owner, dateCreated, dateModified, dateDeleted)
    VALUES (_address, _name,id, _symbol,_type, _owner, _dateCreated, _dateModified, _dateDeleted);
...
CREATE PROCEDURE UpdateContract(IN _address varchar(100), IN _owner varchar(100), IN _dateCreated datetime,
                                IN _dateModified datetime, IN _dateDeleted datetime)
BEGIN
...
CREATE PROCEDURE CreateToken(IN _address varchar(100), INOUT _Id int(11), IN _tokenUri varchar(100), IN _creator varchar(100),
 IN _royalties int(5), IN _minted boolean, IN _amount int(11),IN _soldNum int(11), IN _dateCreated datetime,
                             IN _dateModified datetime, IN _dateDeleted datetime )
BEGIN

     CALL getTokenId(_address, @id);

    SELECT @id INTO _id;


    INSERT INTO tokens (contractAddress, tokenId, tokenUri, creator, royalties, minted, amount, soldNum,dateCreated, dateModified, dateDeleted)
    VALUES (_address, _id, _tokenUri,  _creator, _royalties, _minted, _amount, _soldNum, _dateCreated, _dateModified, _dateDeleted);
...
CREATE PROCEDURE getTokenId(IN _address varchar(100), OUT _Id int(11))

BEGIN

    DECLARE ID INT default 1;

    SELECT MAX(tokenId) FROM tokens where contractAddress = _address into ID;
